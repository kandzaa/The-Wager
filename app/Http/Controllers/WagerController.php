...
class WagerController extends Controller
{
    ...
    public function end(Request $request, Wager $wager)
    {
        ...
        try {
            ...
            $winningChoice    = WagerChoice::findOrFail($validated['winning_choice_id']);
            ...
            if ($totalWinningBets > 0) {
                ...
                foreach ($bets as $bet) {
                    ...
                    if ($bet->choice_id == $winningChoice->id) {
                        $userBets[$userId]['winning_bet'] += $bet->bet_amount;
                    }

                    // Update bet status
                    $bet->update([
                        'status'        => $bet->choice_id == $winningChoice->id ? 'won' : 'lost',
                        'actual_payout' => $bet->choice_id == $winningChoice->id ?
                ...
                foreach ($userBets as $userId => $userBet) {
                    if ($userBet['winning_bet'] > 0) {
                        ...
                        Transaction::create([
                            ...
                            'balance_after' => $userBet['user']->balance + $profit,
                        ]);
                    }
                }
            }
