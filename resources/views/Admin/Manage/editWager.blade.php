...
<div class="min-h-screen bg-slate-50 dark:bg-[#080b0f] py-10">
    <div class="max-w-xl mx-auto px-4 sm:px-6">
        ...
        <div class="flex items-center gap-3 mb-8">
            ...
            <span class="text-slate-500 dark:text-slate-400 text-sm">Edit Wager</span>
        </div>

        {{-- Header --}}
        <div class="mb-8">
            <p class="text-[0.6rem] font-semibold tracking-[0.2em] uppercase text-emerald-500 mb-1">Admin</p>
            <h1 class="text-2xl font-black tracking-tight text-slate-900 dark:text-white">{{ $wager->name }}</h1>
        </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] rounded-xl text-slate-900 dark:text-white text-sm outline-none transition-all duration-200 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 resize-none">{{ old('description', $wager->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_players" value="{{ old('max_players', $wager->max_players) }}" min="2" max="100" required
                            class="w-full h-11 px-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] rounded-xl text-slate-900 dark:text-white text-sm outline-none transition-all duration-200 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20" />
                        @error('max_players') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="status"
                                ...
                                <option value="public"  {{ old('status', $wager->status) === 'public'  ? 'selected' : '' }}>Public</option>
                                <option value="private" {{ old('status', $wager->status) === 'private' ? 'selected' : '' }}>Private</option>
                            </select>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="ending_time"
                            ...
                            required
                            class="w-full h-11 px-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] rounded-xl text-slate-900 dark:text-white text-sm outline-none transition-all duration-200 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20" />
                        @error('ending_time') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
