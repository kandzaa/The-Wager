...
$rarityMeta = [
    ...
    'uncommon'  => ['label' => 'Uncommon',  'text' => 'text-emerald-600 dark:text-emerald-400','bg' => 'bg-emerald-50 dark:bg-emerald-900/30', 'dot' => 'bg-emerald-400'],
    'rare'      => ['label' => 'Rare',      'text' => 'text-blue-600 dark:text-blue-400',      'bg' => 'bg-blue-50 dark:bg-blue-900/30',       'dot' => 'bg-blue-400'],
    'epic'      => ['label' => 'Epic',      'text' => 'text-purple-600 dark:text-purple-400',  'bg' => 'bg-purple-50 dark:bg-purple-900/30',   'dot' => 'bg-purple-400'],
    'legendary' => ['label' => 'Legendary', 'text' => 'text-amber-600 dark:text-amber-400',    'bg' => 'bg-amber-50 dark:bg-amber-900/30',     'dot' => 'bg-amber-400'],
];
$typeMeta = [
...
<div
    class="min-h-screen bg-[#080b0f] py-10"
    x-data="{
        filter: 'all',
        open: false,
        isEdit: false,
...
>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl font-black tracking-tight text-white">Customizations</h1>
                <p class="text-sm text-slate-500 mt-1">{{ $cosmetics->count() }} items across {{ $cosmetics->pluck('type')->unique()->count() }} types</p>
            </div>
            <button @click="openCreate()"
                class="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold rounded-xl transition-all duration-200 active:scale-95 shadow-lg shadow-emerald-900/30">
        ...
        {{-- Filter tabs --}}
        <div class="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/[0.06] rounded-xl w-fit">
            ...
            <button @click="filter = '{{ $val }}'"
                :class="filter === '{{ $val }}' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'text-slate-500 hover:text-slate-300 border-transparent'"
        ...
        <div class="bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-white/[0.06] text-[0.65rem] uppercase tracking-[0.15em] text-slate-500">
                ...
                <tbody class="divide-y divide-white/[0.04]">
                    ...
                    >
                        ...
                        <td class="px-5 py-3.5">
                            @if($c->type === 'frame')
                                <div class="w-8 h-8 rounded-full" style="background: {{ $meta['gradient'] ?? '#888' }}; padding: 2px">
                                    <div class="w-full h-full rounded-full bg-[#080b0f]"></div>
                                </div>
                            @elseif($c->type === 'title')
                        ...
                        <td class="px-4 py-3.5 hidden sm:table-cell">
                            @php $t = $typeMeta[$c->type] ?? ['label'=>$c->type,'text'=>'text-slate-400','bg'=>'bg-slate-800']; @endphp
                            <span class="px-2 py-0.5 rounded-md text-xs font-bold {{ $t['bg'] }} {{ $t['text'] }}">{{ $t['label'] }}</span>
                        </td>
                        {{-- Rarity --}}
                        <td class="px-4 py-3.5 hidden md:table-cell">
                            @php $r = $rarityMeta[$c->rarity] ?? ['label'=>$c->rarity,'text'=>'text-slate-400','bg'=>'bg-slate-800','dot'=>'bg-slate-400']; @endphp
                        ...
                        <td class="px-4 py-3.5">
                            <span class="font-bold text-amber-400">{{ number_format($c->price) }}</span>
                            <span class="text-slate-600 text-xs ml-0.5">c</span>
                        </td>
                        {{-- Key --}}
                        <td class="px-4 py-3.5 hidden lg:table-cell">
                            <code class="text-xs text-slate-500 font-mono">{{ $c->key }}</code>
                        </td>
                        {{-- Actions --}}
                        <td class="px-5 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <button
                                    ...
                                    class="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/[0.07] transition-all">
                                ...
                                <form action="{{ route('admin.Manage.customizations.destroy', $c->id) }}" method="POST"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit" class="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:text-red-400 hover:bg-red-900/20 transition-all">
    ...
    >
        <div
            ...
            @click.stop
            class="w-full max-w-lg bg-[#0d1117] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden"
        >
            {{-- Modal header --}}
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
                <div>
                    <h2 class="text-base font-black text-white" x-text="isEdit ? 'Edit Item' : 'New Item'"></h2>
                    <p class="text-xs text-slate-500 mt-0.5" x-text="isEdit ? 'Update cosmetic details' : 'Add a new cosmetic to the shop'"></p>
                </div>
                <button @click="open = false" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/[0.06] transition-all">
            ...
            <form method="POST" :action="formAction" class="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="type" x-model="form.type" @change="autoKey()"
                            class="w-full px-3 py-2 rounded-xl text-sm bg-black/40 border border-white/[0.08] text-white focus:outline-none focus:border-emerald-500/50 transition-all">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="name" x-model="form.name" @input="autoKey()" placeholder="Gold Frame" required
                            class="w-full px-3 py-2 rounded-xl text-sm bg-black/40 border border-white/[0.08] text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500/50 transition-all"/>
                    </div>
                    <div>
                ...
                {{-- Key --}}
                <div>
                    <label class="block text-[0.65rem] uppercase tracking-[0.15em] font-bold text-slate-500 mb-1.5">Key <span class="normal-case font-normal text-slate-600">(unique identifier)</span></label>
                    <input type="text" name="key" x-model="form.key" required
                        class="w-full px-3 py-2 rounded-xl text-sm bg-black/40 border border-white/[0.08] text-white font-mono focus:outline-none focus:border-emerald-500/50 transition-all"/>
                </div>
                ...
                <div x-show="form.type === 'frame'" class="space-y-3">
                    <div>
                        ...
                        <input type="text" name="meta_gradient" x-model="form.meta_gradient"
                            ...
                            class="w-full px-3 py-2 rounded-xl text-sm bg-black/40 border border-white/[0.08] text-white font-mono placeholder-slate-700 focus:outline-none focus:border-emerald-500/50 transition-all"/>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full flex-shrink-0 ring-2 ring-white/10" :style="`background: ${form.meta_gradient}; padding: 3px`">
                            <div class="w-full h-full rounded-full bg-[#0d1117]"></div>
                        </div>
                        <p class="text-xs text-slate-600">Live frame preview</p>
                    </div>
                </div>
                ...
                <div x-show="form.type === 'title'" class="space-y-3">
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button type="button"
                                ...
                                :class="form.meta_color === '{{ $p['color'] }}' ? 'ring-2 ring-white/40 scale-110' : 'ring-1 ring-white/10'"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="meta_color" x-model="form.meta_color" placeholder="text-emerald-400"
                                class="w-full px-3 py-2 rounded-xl text-xs bg-black/40 border border-white/[0.08] text-white font-mono placeholder-slate-700 focus:outline-none focus:border-emerald-500/50 transition-all"/>
                        </div>
                        <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <p class="text-xs text-slate-600">Live title preview</p>
                    </div>
                </div>
                ...
                <div x-show="form.type === 'theme'" class="space-y-3">
                    ...
                    <div>
                        <label class="block text-[0.65rem] uppercase tracking-[0.15em] font-bold text-slate-500 mb-1.5">BG Class <span class="normal-case font-normal text-slate-600">(CSS class for profile bg)</span></label>
                        <input type="text" name="meta_bg_class" x-model="form.meta_bg_class" placeholder="bg-midnight"
                            class="w-full px-3 py-2 rounded-xl text-sm bg-black/40 border border-white/[0.08] text-white font-mono placeholder-slate-700 focus:outline-none focus:border-emerald-500/50 transition-all"/>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-16 h-8 rounded-lg flex-shrink-0" :style="`background: ${form.meta_gradient}`"></div>
                        <p class="text-xs text-slate-600">Live theme preview</p>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="text" name="meta_emoji" x-model="form.meta_emoji" maxlength="4" placeholder="⭐"
                                class="w-24 px-3 py-2 rounded-xl text-xl bg-black/40 border border-white/[0.08] text-white text-center focus:outline-none focus:border-emerald-500/50 transition-all"/>
                ...
                <div class="flex items-center justify-end gap-2 pt-2 border-t border-white/[0.06]">
                    <button type="button" @click="open = false"
                        class="px-4 py-2 rounded-xl text-sm font-semibold text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all">
                        Cancel
                    </button>
                    <button type="submit"
